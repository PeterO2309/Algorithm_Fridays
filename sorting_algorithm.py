# Source: https://realpython.com/sorting-algorithms-python/#:~:text=mere%2010%20seconds.-,Analyzing%20the%20Strengths%20and%20Weaknesses%20of%20Quicksort,Quicksort%20is%20straightforward%20to%20parallelize.
...
def insertion_sort(array):
    ...
    for i in range(1, len(array)):
        ...
        # the correct position of the element referenced by `key_item`. Do this only
        # if `key_item` is smaller than its adjacent values.
        while j >= 0 and array[j] > key_item:
            # Shift the value one position to the left and reposition j to point to 
            # the next element (from right to left)
            array[j + 1] = array[j]
            j -= 1
